...
| Validation | Zod + @hono/zod-validator | Type-safe request validation, auto-infer types |
...
highlights {
  ...
  UNIQUE (book, chap, sxp, so, exp, eo)  // dedup by position
}
```
...
reading_progress {
  ...
  time_spent   INT   DEFAULT 0       // tổng giây đã đọc (server increments)
  last_read    INT                   // unix timestamp

  PRIMARY KEY (book, chap)
}
// Semantics: PUT gửi { delta_s: 30 } → server: SET time_spent = time_spent + delta_s
```
...
study_log {
  ...
  local_date   TEXT  NOT NULL        // 'YYYY-MM-DD' — client gửi local date (tránh TZ issues)
  created_at   INT   NOT NULL        // unix timestamp (server time)
}
// Client gửi local_date theo timezone user (new Date().toISOString().slice(0,10))
// Group by local_date cho streak calculation
```
...
questions {
  ...
  question     TEXT  NOT NULL        // Nội dung câu hỏi (searchable, tách ra khỏi JSON)
  options      TEXT                  // JSON array: ["A. ...", "B. ..."] (mc only, null for essay)
  answer       TEXT  NOT NULL        // Đáp án: index number (mc) hoặc text (essay)
  explain_vi   TEXT                  // Giải thích tiếng Việt
  explain_en   TEXT                  // Explanation in English
  difficulty   INT   DEFAULT 3       // 1–5 (static, import time)
  tags         TEXT                  // JSON array: ['agile', 'scope', ...]
  created_at   INT   NOT NULL
}
// question column tách riêng để support full-text search (FTS5 Phase 4)
```
...
```typescript
question_stats {
  id           TEXT  PRIMARY KEY     // UUID — cho phép retry cùng câu trong 1 session
...
// Index: CREATE INDEX idx_qstats_question ON question_stats(question_id)
// Index: CREATE INDEX idx_qstats_session ON question_stats(session_id)
```
Query "câu yếu": `GROUP BY question_id → AVG(correct) ASC`
Practice mode cho phép retry → cùng (question_id, session_id) có thể xuất hiện nhiều lần.
...
**SRS Query behavior:** `WHERE next_review <= unixepoch()` trả về tất cả overdue cards.
Sort by `next_review ASC` (oldest overdue first) để ưu tiên cards bị miss lâu nhất.

### 3.10 Full-Text Search (Phase 4)
```sql
-- Virtual table cho search vocab + questions
CREATE VIRTUAL TABLE vocab_fts USING fts5(en, vi, desc_vi, content=vocab_terms, content_rowid=rowid);
CREATE VIRTUAL TABLE questions_fts USING fts5(question, content=questions, content_rowid=rowid);
```
...
GET    /*                                   serve static files từ pmp/ root
                                            (PHẢI đặt sau /api/* routes)
```

### Error Response Contract
```typescript
// Mọi error response đều có format:
{ error: string, code?: string }

// HTTP Status Codes:
// 400 — Validation error (Zod parse fail, missing required fields)
// 404 — Resource not found
// 500 — Server error (DB failure, unexpected)

// Ví dụ:
// PUT /api/highlights/xxx với body thiếu 'color':
// → 400 { error: "color is required", code: "VALIDATION_ERROR" }
```
...
**SQLite PRAGMA (trong `src/db/client.ts`):**
```typescript
const db = new Database(process.env.DB_PATH);
db.pragma('journal_mode = WAL');       // concurrent reads while writing
db.pragma('foreign_keys = ON');        // enforce FK constraints
db.pragma('busy_timeout = 5000');      // retry 5s khi locked
db.pragma('synchronous = NORMAL');     // balance speed vs durability
```

**Seed script phải idempotent:**
```typescript
// Dùng INSERT OR IGNORE / ON CONFLICT DO NOTHING
// Chạy nhiều lần không tạo duplicate
db.run(`INSERT OR IGNORE INTO vocab_terms (id, en, vi, ...) VALUES (?, ?, ?, ...)`);
```
...
SQLite = 1 file → backup đơn giản:

```bash
# Cron job daily trên Oracle Cloud (hoặc chạy tay local)
mkdir -p data/backups
cp data/pmp.db "data/backups/pmp-$(date +%Y%m%d-%H%M).db"

# Giữ 30 ngày gần nhất, xóa cũ hơn
find data/backups -name '*.db' -mtime +30 -delete
```

Khi deploy Oracle Cloud: thêm vào crontab `0 3 * * * /path/to/backup.sh`

---

## 12. Route Ordering (quan trọng)

Trong `src/index.ts`, thứ tự middleware/routes:

```typescript
// 1. CORS middleware (dev only)
app.use('/api/*', cors());
...
// 3. Static file serving LAST (fallback)
app.use('/*', serveStatic({ root: '../' }));
```

Nếu static serve trước → file trùng path `/api/` sẽ shadow API routes.
