...
(function () {
  ...
  if (!document.getElementById('hl-system-css')) {
    ...
    css.textContent = [
      ...
      '#hl-mob-hl{flex:1.3;background:rgba(201,168,76,.1);border:1px solid rgba(201,168,76,.4);color:#c9a84c;border-radius:8px;height:38px;font-size:.78rem;font-family:\'Source Sans 3\',sans-serif;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:.3rem;white-space:nowrap}',
      '#hl-mob-badge{background:rgba(201,168,76,.25);color:#c9a84c;font-size:.65rem;font-weight:700;padding:.1rem .35rem;border-radius:10px;min-width:18px;text-align:center}',
  ...
  (function () {
    ...
    function updateBadge() {
      ...
      var mobBadge = document.getElementById('hl-mob-badge');
    ...
    (function () {
      ...
      bar.innerHTML =
        ...
        + '<button id="hl-mob-hl">✏ <span id="hl-mob-badge">' + loadHL().length + '</span></button>'
        + '<div id="hl-mob-nav">'
      ...
      document.getElementById('hl-mob-hl').addEventListener('click', function () { openPanel(); });
    }());
    ...
    function hidePopup() { popup.classList.add('hidden'); pendingRange = null; pendingXP = null; currentHlId = null; }
    ...
    // Mobile: touchend (fires when finger lifts after text selection)
    document.addEventListener('touchend', function (e) {
      if (popup.contains(e.target) || noteModal.contains(e.target) || panel.contains(e.target)) return;
      setTimeout(processSelForPopup, 300);
    });

    // Mobile: selectionchange with debounce (handles drag-select handle adjustments)
    var _selTimer = null;
    document.addEventListener('selectionchange', function () {
      clearTimeout(_selTimer);
      _selTimer = setTimeout(function () {
        if (!popup.classList.contains('hidden')) return; // popup already open, don't re-trigger
        var sel = window.getSelection();
